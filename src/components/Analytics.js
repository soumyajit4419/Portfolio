...
const Analytics = (props) => {
  ...
  const fetchApi = async () => {
    const dailyData = await fetch(`${process.env.REACT_APP_PRODUCTION_ANALYTICS_URL && process.env.NODE_ENV === 'production' ? process.env.REACT_APP_PRODUCTION_ANALYTICS_URL : "http://localhost:8080/"}getdata`, {
      headers: { 
  ...
  // const Linedata =  ;
    ...
    return (
      <div>
            <Container fluid className="resume-section" style={{ height: `${1860 + window.innerHeight}px` }}>
        <Container>
          {/* <Row> */}
        ...
        <br />
{
       ...
       <Row>
       <Col md={6}> 
       <Pie width={10} height={10}  options={{
          responsive: true,
...
return <li key={i}>
  {e.ip} - {e.reason}
         ...
         }}> block ip </button> &nbsp;  <button onClick={() => {
          let ip = prompt("IP:");
          let reason = prompt("Reason:") || undefined;
          if(!ip) return alert("Please enter an IP address")
       ...
       <h1> Refferers</h1>
         ...
         <Doughnut data={{
      backgroundColor: "red",
      labels: dailyData.refs?.label.map(v => v === null ? "None" : v),
      datasets: [
        {
          label: "# requests ",
...
<ul>
  {
    dailyData.refs?.label.map((label, index) => {
      ...
      return (<li key={index}>
        {label} - {amount} <br /> <button onClick={() => {
      var url = new URL(window.location.href);
      var search_params = url.searchParams;
      
      // new value of "id" is set to "101"
     if(label != "No Ref") search_params.set('ref', label);
...
<br />
   <ul> {dailyData.connectedClients?.map((id) => {
      return (
        <li key={id}> 
        {id} 
        </li>
 ...
 setEvalValue(value)
    ...
    <button onClick={() => {
      ...
      fetch(`${process.env.REACT_APP_PRODUCTION_ANALYTICS_URL && process.env.NODE_ENV === 'production'  ? process.env.REACT_APP_PRODUCTION_ANALYTICS_URL : "http://localhost:8080/"}clients/${id}/eval`, {
        headers:{
          "Content-Type": "application/json",
          "Authorization": process.env.REACT_APP_PASSWORD
        },
...
<Col md={6}>
  <button onClick={fetchApi}> Re-fetch (sockets may need more then 1 reload)</button>
</Col>
       </>
         : <></>
}        </Container>
